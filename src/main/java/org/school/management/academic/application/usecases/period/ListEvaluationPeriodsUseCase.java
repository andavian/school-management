...
public class ListEvaluationPeriodsUseCase {

    private final EvaluationPeriodRepository evaluationPeriodRepository;
    private final AcademicYearRepository academicYearRepository;
    private final AcademicApplicationMapper mapper;

    public List<EvaluationPeriodResponse> execute(String academicYearIdStr) {
        log.debug("Listing evaluation periods for academic year: {}", academicYearIdStr);
        ...
        List<EvaluationPeriod> periods = evaluationPeriodRepository.findByAcademicYear(academicYearId);

        return periods.stream()
                .map(mapper::toEvaluationPeriodResponse)
